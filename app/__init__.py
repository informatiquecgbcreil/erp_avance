...
def create_app():
    ...
    # ---------------------------------------------------------------------
    # Some templates are shared across legacy/new modules and may reference
    # endpoints that are not registered in a given deployment. This helper
    # prevents hard crashes (BuildError) by returning a fallback URL.
    ...
    def ensure_schema():
        ...
        # 4b) Activité : traçabilité / cloisonnement participants (création en avance)
        try:
            cols_p = [row[1] for row in db.session.execute(text("PRAGMA table_info(participant)")).all()]
            alters = []
            if "created_by_user_id" not in cols_p:
                alters.append("ALTER TABLE participant ADD COLUMN created_by_user_id INTEGER REFERENCES user(id)")
            if "created_secteur" not in cols_p:
                alters.append("ALTER TABLE participant ADD COLUMN created_secteur VARCHAR(80)")
            for sql in alters:
                db.session.execute(text(sql))
            if alters:
                db.session.commit()
